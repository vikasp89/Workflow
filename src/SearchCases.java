    ...
    try {
      ...
      if (searchType.equals("Advanced")) {
        ...
        if (treating_dr_email == "")
        	treating_dr_email = "(!#)"; 
          String query = " select * from cc_crm where Case_Id like '%" + caseId + "%' or Patient_Name like '%" + 
      ...
      if (searchType.equals("Moderate")) {
        ...
        String query = " select * from cc_crm where CREATED_DATE between '" + to + "' and '" + from + 
      ...
      if (searchType.equals("Simple")) {
        String query = " select * from cc_crm order by CREATED_DATE desc limit " + days;
        stmt = connection.prepareStatement(query);
        rs = stmt.executeQuery();
        while (rs.next()) {
          String phone = "";
